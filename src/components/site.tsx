...
export async function SiteFooter({ locale }: { locale: string }) {
  ...
  return <footer className="mt-16 border-t border-border bg-card/30"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><div className="mb-10 rounded-2xl border border-border bg-muted/40 p-5"><div className="font-bold text-foreground">{siteConfig.name}</div><p className="mt-1 text-sm text-muted-foreground">{t("description")}</p>{siteConfig.gameUrl && <Link href={siteConfig.gameUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[hsl(var(--nav-theme))]">{t("playGame")} <ExternalLink className="h-4 w-4" /></Link>}</div><p className="mb-8 text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">{site("legalNotice")}</p><div className="grid gap-8 md:grid-cols-4"><div className="md:col-span-2"><h3 className="font-bold text-foreground">{t("aboutTitle")}</h3><p className="mt-3 max-w-xl text-sm leading-7 text-muted-foreground">{t("about")}</p>{siteConfig.supportEmail && <p className="mt-3 text-sm text-muted-foreground">Contact: <a href={`mailto:${siteConfig.supportEmail}`} className="underline hover:text-foreground">{siteConfig.supportEmail}</a></p>}</div><FooterList locale={locale} title={t("quickLinks")} links={[[t("playGame"), siteConfig.gameUrl || "#"], [t("officialDiscord"), siteConfig.social?.discord || "#"], [t("officialYoutube"), siteConfig.social?.youtube || "#"]]} /><FooterList locale={locale} title={t("guides")} links={[[t("beginnerGuide"), "/guide"], [t("raceGuides"), "/races"], [t("bossGuides"), "/bosses"], [t("buildGuide"), "/guide"], [t("privacyPolicy"), "/privacy-policy"], [t("termsOfService"), "/terms-of-service"]]} /></div><p className="mt-10 border-t border-border pt-6 text-xs text-muted-foreground">{t("copyright")}</p></div></footer>;
}
